...
class IdentitiyBlock_3(tf.keras.Model):
    ...
    Args:
        ...
        kernel_sizes : list of 3 integers, which are kernel size of convolution filters
    """
    def __init__(self,
                 filters,
                 kernel_sizes,
        ...
        self.conv1 = tf.layers.Conv1D(filters[0], kernel_sizes[0], padding="same", activation=tf.nn.relu)
        # self.bn1 = tf.layers.BatchNormalization()

        self.conv2 = tf.layers.Conv1D(filters[1], kernel_sizes[1], padding="same", activation=tf.nn.relu)
        # self.bn2 = tf.layers.BatchNormalization()

        self.conv3 = tf.layers.Conv1D(filters[2], kernel_sizes[2], padding="same", activation=tf.nn.relu)
...
class ConvolutionBlock_3(tf.keras.Model):
    ...
    Args:
        ...
        kernel_sizes : list of 4 integers. last size is for shortcut
    """
    def __init__(self,
                 filters,
                 kernel_sizes,
        ...
        self.conv1 = tf.layers.Conv1D(filters[0], kernel_sizes[0], strides=2, padding="same",
        ...
        self.shortcut = tf.layers.Conv1D(filters[2], kernel_sizes[3], strides=2, padding="same",
                                         activation=tf.nn.relu)
